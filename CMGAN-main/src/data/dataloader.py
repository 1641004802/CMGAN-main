...
def load_data(ds_dir, batch_size, n_cpu, cut_len):
    ...
    # train_dataset = DataLoader(dataset=train_ds, batch_size=batch_size, shuffle=True,
    #                                             drop_last=True, num_workers=n_cpu)
    # test_dataset = DataLoader(dataset=test_ds, batch_size=batch_size, shuffle=False,
    #                                            drop_last=False, num_workers=n_cpu)
    # return train_dataset, test_dataset
    #修改2------
    train_sampler = DistributedSampler(train_ds)
    test_sampler = DistributedSampler(test_ds)
    train_loader=DataLoader(train_ds, batch_size=batch_size, sampler=train_sampler,drop_last=True, num_workers=n_cpu)
    test_loader=DataLoader(test_ds, batch_size=batch_size, sampler=test_sampler,drop_last=False, num_workers=n_cpu)
